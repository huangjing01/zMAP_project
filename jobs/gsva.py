...
def prepare_gsva_kegg(z_statistic_matrix,sample_info,gmt,outdir,type_="kegg",top_n=50,adj_P_Val=0.05):
    ...
    out.write('group_info <- read.csv("%s",sep="\\t")\n'%sample_info)
...
if __name__=="__main__":
    ...
    parser.add_argument("--adj_P_Val",help="adj_P_Val cutoff",type=float,default=0.05)
    ...
    adj_P_Val = float(argv['adj_P_Val'])
    kegg_gmt = "/mnt/storage/user/guixiuqi/rsgeno/protein/zmap/zmap_2023_9_13/enrichr.KEGG_2016.gmt"
    prepare_gsva_kegg(z_statistic_matrix,sample_info,kegg_gmt,outdir,type_="kegg",top_n=50,adj_P_Val=adj_P_Val)
    go_gmt = '/mnt/storage/user/guixiuqi/rsgeno/protein/zmap/zmap_2023_9_13/human_GO_Biological_Process_2015.gmt'
