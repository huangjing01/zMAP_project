...
@QC.route('/', methods=['GET', 'POST'])
def index():
    return render_template('QC_index.html')


@QC.route('/result', methods=['POST'])
def result():
    ...
    #    print(color_df)
        log_info.append("Performing sample-based hierarchical clustering.")
        cluster_(correlation_df,color_df,outdir,condition_color_dict)

        log_info.append("Performing principal component analysis")
        ...
        return render_template('QC_result.html',log_info=log_info,filedir_id=proc_id+".zip")
    else:
...
@QC.route('/result/<filedir>', methods=['GET'])
