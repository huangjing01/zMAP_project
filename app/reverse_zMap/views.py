...
from . import reverse_zMap
from jobs.reverse_zMap import async_reverse_zMap
...
@reverse_zMap.route('/example/<example_file>', methods=['GET', 'POST'])
def example_data(example_file):
    return send_from_directory(os.path.abspath(os.path.join(current_app.config['UPLOAD_FOLDER'],'example')),example_file,as_attachment=True)


@reverse_zMap.route('/result/<process_id>', methods=['GET', 'POST'])
def result(process_id):  
    if process_id[-4:] == '.zip':
        ...
        with open("reverse_zMAP_input.info.log","r") as g:
            a = json.load(g)
        g.close()
        task_id = a["task_id"]
        email = a["email"]
        return render_template("reverse_zMap_result.html",task_id=task_id,email=email)

@reverse_zMap.route('/image/<process_id>/<image_query>', methods=['GET', 'POST'])
def image_cls(process_id,image_query):
    ...
    if image_query == "normalization":
        ...
        for path, dir, filelist in os.walk(os.getcwd()):
            for file_name in filelist:
                if file_name.endswith("distribution_r2_intercept_of_linear_regression.png"):
                    image_list.append(file_name)
        return jsonify(error="",id=image_list)
...
@reverse_zMap.route('/result/<process_id>/<image>', methods=['GET', 'POST'])
...
@reverse_zMap.route('/', methods=['GET', 'POST'])
def index():
    return render_template('reverse_zMap_index.html')

#file content should be validated
@reverse_zMap.route('/validation/<input_type>', methods=['POST'])
...
@reverse_zMap.route('/reverse_zMap_processing/<process_id1>/<process_id2>', methods=['POST'])
def reverse_zMap_processing(process_id1,process_id2):
    ...
    if request.form.get('window_size') and request.form.get('step_size') and request.form.get('percent') and request.form.get('method'):
        ...
        task = async_reverse_zMap.apply_async(kwargs={
        ...
        reverse_zMap_input='reverse_zMAP_input.info.log'
        ...
        with open(reverse_zMap_input,"w") as f:
            json.dump(d,f)
        f.close()
        return redirect(url_for("reverse_zMap.result",process_id=process_id1))
    else:
        flash('please confirm modeling parameters.')
        return redirect(url_for('reverse_zMap.index'))



@reverse_zMap.route('/status/<process_id>/<task_id>',methods=["POST","GET"])
def taskstatus(process_id,task_id):
    task = async_reverse_zMap.AsyncResult(task_id)
    ...
    if task.state == 'PENDING':
        ...
        if response['status'][-1] == 'reverse_zMap finished':
