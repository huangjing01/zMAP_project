...
import zipfile
from .reverse_zmap_step1 import *
import numpy as np
import json,random
...
def async_reverse_zMap(self,intensity_file,sample_info_file,window_size,step_size,percent,method,email):
    try:
        with time_limit(60*60*6):
            ...
            reverse_zMap_status.append("data qc...")
            self.update_state(state='PROGRESS',meta={'status':reverse_zMap_status})
            ...
            reverse_zMap_status.append("start one vs one comparison...")
            ...
            for paired_df in paired_df_list:
                ...
                reverse_zMap_status.append(sample + ' : model with ' + str(percent * 100) + '% proteins of each sliding window...')
                self.update_state(state='PROGRESS',meta={'status': reverse_zMap_status})
                ...
                if final_method == "natural_cubic_spline":
                    nonlinear_model,R_square = fit_natural_model(log2_normalized_intensity_df,all_window_mean_value_list,\
                                        all_window_variance_list,used_for_model_fitting=[0,1])
                    ...
                    popt,exp_R_square = fit_exp_model(log2_normalized_intensity_df,all_window_mean_value_list,all_window_variance_list,used_for_model_fitting=[0,1])
                    ...
                    if exp_R_square > 0.5:
                        ...
                        log2_normalized_intensity_df["estimated variance"] = \
                        [np.mean(all_window_variance_list)] * len(log2_normalized_intensity_df)
                reverse_zMap_status.append(sample + ' : calculate normalization factor bias...')
                self.update_state(state='PROGRESS',meta={'status': reverse_zMap_status})
            ...
            reverse_zMap_status.append("combine all sample results...")
            self.update_state(state='PROGRESS',meta={'status':reverse_zMap_status})
