...
def send_async_email(filepath,email):
    ...
    message['Passwd'] = 'AGMFDPTTBRHBSFGO'
    os.chdir(filepath[:-4])
    ...
    try:
        with time_limit(60*60*6):
            ...
            for ms_run in ms_run_list:
                ...
                one_zMap_info.append(batch + ' : VA-plot of all proteins with p-values...')
                self.update_state(state='PROGRESS',meta={'status': one_zMap_info})
                pvalue_scatterplot(zmap_result_df,result_dir=output_dir,batch=batch)
                
                one_zMap_info.append(batch + ' : plot modeling performance...')
                self.update_state(state='PROGRESS',meta={'status': one_zMap_info})
        ...
        for one_task in all_one_zMap_task:
